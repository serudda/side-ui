...
import { useDocumentEvent } from '../useDocumentEvent/useDocumentEvent';
...
export const useOnClickOutside = (
  ...
  function handleOutsideClick<E extends MouseEvent | PointerEvent | FocusEvent>(
    ...
    const _containers = (function resolve(containers): ContainerCollection {
      if (typeof containers === 'function') return resolve(containers());
    ...
    for (const container of _containers) {
      ...
      if (domNode?.contains(target)) return;
      ...
      if (event.composed && event.composedPath().includes(domNode as EventTarget)) return;
    }
